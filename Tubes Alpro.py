...
def about_me():
    msg.showinfo("About Me", 
                 ...
                 "Anggota kelompok :\n"
                 "Sania Dwi Ayu Lestari_123450086\n"
                 "Afifah Fauziah_123450002\n"
                 "Aprilia Dewi Hutapea_123450040\n"
                 "Muhammad Naufal Alghani_123450116\n"
                 "Selamat mencoba program quiz kami!")

def send_feedback():
    tanya = msg.askquestion('Feedback', 'Was your experience good with us?')
    if tanya == 'yes':
        msg.showinfo('Feedback', "Thank you for using our quiz application! 😊\n"
                                 "We truly appreciate your support. If you have any suggestions or feedback, "
                                 "please feel free to contact us via email. We hope this application has been helpful.\n"
                                 "Thank you and have a great day!")
    else:
        msg.showwarning('Feedback', "We apologize if your experience was not satisfactory. "
                                   "We would greatly appreciate any constructive criticism or suggestions. "
                                   "Please feel free to reach out to us via email to share your feedback. Thank you.")

def exit_app():
    if msg.askyesno("Exit", "Are you sure you want to exit the application?"):
        root.quit()
...
def add_question(window, username):
    ...
    option_vars = {opt: tk.StringVar() for opt in ["A", "B", "C", "D"]}
...
def show_add_question_form(matkul, add_window, username):
    ...
    entry_question.pack(pady=10)
      ...
      # Input opsi jawaban
    tk.Label(add_window, text="Opsi Jawaban:", font=("Times New Roman", 14), bg="#081F5C", fg="#F7F2EB").pack(pady=5)
    for opt in ["A", "B", "C", "D"]:
        frame = tk.Frame(add_window, bg="#081F5C")
        frame.pack(pady=2)
        tk.Label(frame, text=f"{opt}:", font=("Times New Roman", 12), bg="#081F5C", fg="#F7F2EB").pack(side="left")
        tk.Entry(frame, textvariable=option_vars[opt], font=("Times New Roman", 12), width=30).pack(side="left", padx=5)
...
def start_quiz(matkul, window):
    ...
    tk.Label(quiz_window, text="Selamat datang di Kuis Sains Data Kelompok 1 RC!",
